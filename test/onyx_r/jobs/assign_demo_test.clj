...
(ns onyx-r.jobs.assign-demo-test
...
(deftest assign-demo-test
  (testing "that we can assign R variables through onyx-r"
    (let [{:keys [env-config
                  peer-config]} (read-config (io/resource "config.edn"))
          job (onyx-r.jobs.assign-demo/assign-demo-job {:onyx/batch-size 10
      ...
      (with-test-env [test-env [3 env-config peer-config]]
        ...
        (is (= (set (take-segments! out))
               (set [{:n 3.0} {:n 4.0} {:n 5.0} {:n 6.0} {:n 7.0} :done])))))))
