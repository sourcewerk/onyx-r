...
(ns onyx-r.jobs.source-demo-test
...
(deftest source-demo-test
  (testing "that we can have a basic in-R-out workflow run through onyx-r"
    (let [{:keys [env-config
                  peer-config]} (read-config (io/resource "config.edn"))
          job (onyx-r.jobs.source-demo/source-demo-job {:onyx/batch-size 10
